...
BEGIN
    ...
    IF p_hora_reserva < '10:00:00' THEN
        RETURN 10; -- Retornar 10 si la hora es antes de las 10:00:00
    ELSIF p_hora_reserva > '20:00:00' THEN
        RETURN 20; -- Retornar 20 si la hora es después de las 20:00:00
    END IF;
    ...
    IF EXISTS (
        ...
        -- Retornar 15 si hay un empalme en las citas
        RETURN 15; 
    END IF;

    -- Seleccionar un barbero disponible al azar
    SELECT usuario_barbero INTO v_barbero_usuario
    FROM barbero b
    WHERE NOT EXISTS (
        SELECT 1 
        FROM cita c 
        WHERE c.nombre_barbero = b.usuario_barbero 
        AND c.fecha = p_fecha 
        AND c.hora_reserva = p_hora_reserva
    )
    ORDER BY RANDOM() -- Ordenar aleatoriamente
    LIMIT 1; -- Tomar solo uno

    -- Verificar si hay un barbero disponible
    IF v_barbero_usuario IS NULL THEN
    ...
    VALUES (p_usuario_cliente, p_fecha, p_hora_reserva, v_costo_total, v_barbero_usuario, p_metodo_pago, v_costo_total / 2)
    ...
    -- Calcular la hora de término como 5 minutos multiplicado por la cantidad de servicios
    ...
    IF v_hora_termino > '20:00:00' THEN
        RETURN 20; -- Retornar 20 si excede las 20:00:00.
    END IF;
    ...
    RETURN v_id_cita; -- Retornar el ID de la cita creada
END;
$$
 LANGUAGE plpgsql;
--
--
SELECT Agregar_Servicio_Cita(1, 1); SELECT Registrar_Cita('client1', '2024-09-27', '13:40:00', 'Tarjeta'); 
--
SELECT Agregar_Servicio_Cita(2, 1); SELECT Registrar_Cita('client2', '2024-09-27', '13:41:00', 'Tarjeta'); 
--
SELECT Agregar_Servicio_Cita(3, 1); SELECT Registrar_Cita('client3', '2024-09-27', '13:42:00', 'Tarjeta'); 
--
SELECT Agregar_Servicio_Cita(3, 1); SELECT Registrar_Cita('client4', '2024-09-27', '13:43:00', 'Tarjeta'); 
--
SELECT Agregar_Servicio_Cita(1, 2); SELECT Registrar_Cita('client1', '2024-09-27', '13:50:00', 'Tarjeta'); 
--
SELECT Agregar_Servicio_Cita(1, 3); SELECT Agregar_Servicio_Cita(2, 4); SELECT Agregar_Servicio_Cita(3, 2); 
SELECT registrar_cita('client3', '2024-09-27', '14:50:00', 'Tarjeta');
--select * from cita
